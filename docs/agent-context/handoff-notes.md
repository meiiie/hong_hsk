...
A stacked local-only branch, `codex/neko-local-pilot`, adds a post-answer Neko UX pilot without changing the cleanup PR. Vite launches the owner's installed ordinary `neko acp` on demand; same-origin loopback routes pass bounded card/attempt context and stop/close commands into an ACP session running in `plan` mode, low effort, one stable empty workspace, no configured MCP, no outside reads, and no extra verification loop. A floating Neko launcher opens a non-layout-shifting conversation panel; during recall the panel shows only the pedagogical lock, while history and questions appear only after check/reveal. Production builds omit the tutor dependency and Vite middleware, so this branch is for local product evaluation, not deployment.
...
The branch also upgrades the developer-local Neko surface to a continuous conversation. One ACP session ID is reused across cards and page reloads; Neko remains the durable-session and full-context authority, while the browser keeps a bounded presentation cache so the panel can immediately redraw recent messages. Storage/transport implementation copy is intentionally absent from the primary chat UI. The learner can stop a pending answer, retry, turn AI off/on without affecting study, export JSON, or start a new conversation with a two-step confirmation. Starting new forgets the browser presentation cache and calls ACP `session/close`; it does not claim to delete Neko's durable file because ACP v1 has no delete call. ACP text deltas are coalesced at the bridge into readable blocks: a block flushes at a sentence boundary, around 120 characters, or after at most 140 ms. The loopback route streams those blocks as NDJSON, and the panel patches only the active answer while preserving Stop and avoiding full-app rerenders; completed turns alone enter the presentation cache. Mock desktop/mobile coverage passes for two turns and all controls. On 2026-08-28, installed Neko `v1.3.0` passed `doctor`, a direct `bai/glm-5.3-flash` completion, and two real Hồng HSK4 ACP turns with the same conversation ID. Neko resolves the active profile/model when the ACP child starts; after a non-cancellation turn failure the local bridge now recycles that child so the learner's next explicit retry reloads a newly selected profile/model without silently duplicating a request.
