...
The AI tutor is implemented as a streaming Cloudflare Pages Function at `/api/ai/tutor`. Production needs the Pages secret `NVIDIA_API_KEY`; the default primary model is `mistralai/mistral-nemotron`, with fallback to `nvidia/nemotron-3-super-120b-a12b` for provider timeout/errors so the UI can avoid long blank waits. If both provider models are unavailable or too slow, the gateway emits a short `local-hsk-fallback` answer built only from the current card data; treat it as a learner recovery hint, not verified AI-generated content. `nvidia/nemotron-3-ultra-550b-a55b` is too slow for default study-card turns and should only be used via override for special deep-reasoning experiments. The browser stores a lightweight tutor chat session in localStorage and sends compact markdown memory plus recent messages. AI output is tutor assistance only and must not be treated as verified vocabulary data.
